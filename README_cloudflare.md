...
9. [ ] `.env` / `.dev.vars` の設定
10. [ ] seed（初期データ投入）
11. [ ] ローカル動作確認（`npm run dev` / `npx wrangler dev --remote`）
12. [ ] デプロイ + Secret 登録
13. [ ] （任意）評価スクリプト実行
...
> リポジトリ名が `rag` の場合、ダッシュボード上の Worker 名も `rag` になることが多いです。  
> `wrangler.toml` の `name = "clock-diagnosis-rag"` とは別名になり得ます（後述）。
...
| 経路 | Worker 名の例 | URL 例 | 本 PoC での位置づけ |
|------|---------------|--------|---------------------|
| Create application（GitHub） | `rag` | `rag.watchkarte.workers.dev` | **手順 3（workers.dev 有効化）に使用** |
| `npx wrangler deploy`（CLI） | `clock-diagnosis-rag` | `clock-diagnosis-rag.watchkarte.workers.dev` | 手順 13 の本番デプロイ推奨 |
...
2. D1 `diagnoses` のクリア
3. `src/data/seed.json`（60 件）を D1 へ INSERT
...
そのため **pure local では `POST /diagnose` が成立しません**。  
...
### 12.6 よくあるエラー: `wrangler: コマンドが見つかりません`
...
### 12.7 よくあるエラー: workers.dev サブドメイン未登録
...
デプロイ後の URL 例（`wrangler.toml` の `name` とアカウントサブドメインによる）:

```text
https://clock-diagnosis-rag.watchkarte.workers.dev
```

Git 連携で作った Worker（`rag`）をそのまま使う場合の例:
...
```bash
curl -s https://clock-diagnosis-rag.watchkarte.workers.dev/diagnose \
  -H 'content-type: application/json' \
  -d '{"symptom":"秒針が5秒おきに飛んで止まる"}'
```
...
```bash
DIAGNOSE_API_URL=https://clock-diagnosis-rag.watchkarte.workers.dev/diagnose npm run eval
```
...
| 症状 | 想定原因 | 対処 |
|------|----------|------|
| `wrangler: コマンドが見つかりません` | グローバル `wrangler` が無い | `npm run dev` または `npx wrangler ...` を使う（手順 12.1 / 12.6） |
...
| リソース | 名前（既定） | 用途 |
|----------|--------------|------|
| Workers（CLI） | `clock-diagnosis-rag` | `wrangler deploy` 時の API 本体 |
